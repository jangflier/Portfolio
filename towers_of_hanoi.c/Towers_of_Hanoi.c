...
*
* 하노이의 탑을 재귀함수로 푸는 이유 :
* for문과 if문으로 하노이의 탑을 구현하면 매우 복잡해지지만 재귀함수로 구현할 경우 아래처럼 짧은 코드만으로 구현이 가능해지고 가독성이 좋아집니다.
*/
...
int main(void)
{
	...
	while (1) {
		...
		// -- 목표 기둥 입력 --
		printf("\n2. 원반들의 목표기둥은 몇 번째 기둥입니까?(기둥은 1~3번 까지 있음) : ");
		if ((goal = IntScanf()) == -1) {
			puts("\a정수를 입력해 주세요!!");
			continue;
		}
			...
			Towers_of_Hanoi(num, start, goal);
			printf("옮긴 횟수 : %d회\n", count);
			puts("\n성공적으로 게임을 끝 마쳤습니다.\n");
			break;
		}
